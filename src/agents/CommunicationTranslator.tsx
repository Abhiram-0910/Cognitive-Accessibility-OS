...
import { Loader2, ArrowRight, ArrowLeftRight, BatteryCharging, ShieldCheck } from 'lucide-react';
...
export const CommunicationTranslator: React.FC = () => {
  ...
  const handleTranslateOutbound = async () => {
    ...
    try {
      ...
      setOutboundResult(JSON.parse(result.response.text()));
    } catch (error) {
      console.error(error);
    } finally {
      setLoading(false);
    ...
    <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 w-full max-w-2xl mx-auto mt-8">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex bg-slate-100 p-1 rounded-xl">
          <button 
            onClick={() => { setDirection('inbound'); setOutboundResult(null); }}
            className={`px-4 py-1.5 rounded-lg text-xs font-semibold transition-colors ${direction === 'inbound' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
          >
            Decode Incoming
          </button>
          <button 
            onClick={() => setDirection('outbound')}
            className={`px-4 py-1.5 rounded-lg text-xs font-semibold transition-colors ${direction === 'outbound' ? 'bg-indigo-500 text-white shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
          >
          ...
          {outboundResult && (
            <div className="mt-6 space-y-4">
              <div className="p-4 bg-indigo-50/50 rounded-xl border border-indigo-100 relative">
                <h4 className="text-[10px] font-bold text-indigo-400 uppercase tracking-wider mb-2">Safe to Send</h4>
                <p className="text-sm font-medium text-slate-800">{outboundResult.translated_text}</p>
                ...
                <button 
                  ...
                  onClick={() => navigator.clipboard.writeText(outboundResult.translated_text)}
                >
                  Copy
                </button>
              </div>
              ...
              <div className="flex gap-4">
                <div className="flex-1 p-4 border border-slate-100 rounded-xl">
                  <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2 flex items-center gap-1"><ShieldCheck className="w-3 h-3" /> Adjustments Made</h4>
                  <ul className="text-xs text-slate-600 space-y-1">
                    {outboundResult.tone_adjustments.map((adj, i) => <li key={i}>• {adj}</li>)}
                  </ul>
                </div>
                ...
                <div className="shrink-0 p-4 border border-emerald-100 bg-emerald-50/30 rounded-xl flex flex-col items-center justify-center min-w-[120px]">
                  ...
                  <span className="text-2xl font-light text-emerald-700">{outboundResult.masking_energy_saved_minutes}m</span>
                  <span className="text-[10px] font-bold text-emerald-600 uppercase tracking-wider text-center mt-1">Energy<br/>Saved</span>
                </div>
              </div>
            </div>
          )}
