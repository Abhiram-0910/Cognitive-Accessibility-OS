  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        ...
        // Supabase returns the JSONB array directly on the column
        if (data?.questions) {
    ...
    try {
      ...
      if (user) {
        await supabase.from('kids_sessions').insert({
          ...
          session_name: sessionName || 'Anonymous Explorer',
          game_name: gameName
        });
      }
    } catch (err) {
      console.error("[Game] Failed to create session record", err);
    }
