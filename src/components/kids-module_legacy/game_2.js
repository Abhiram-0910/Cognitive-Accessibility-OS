...
const Game = ({ gameId }) => {
  ...
  useEffect(() => {
    const fetchGameQuestions = async () => {
      try {
        ...
        // Supabase returns the JSONB array directly
        const qData = data?.questions || [];
        setQuestions(qData);
        if (qData.length > 0) {
          setCompletedWord(qData[0]?.word || "");
        }
      } catch (error) {
        console.error("Error fetching game questions from Supabase:", error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const startGame = async () => {
    ...
    try {
      ...
      if (user) {
        await supabase.from('kids_sessions').insert({
          user_id: user.id,
          session_id_legacy: sessionId,
          session_name: username || 'Anonymous Explorer',
          game_name: gameName
        });
      }
    } catch (err) {
      console.error("[Game 2] Failed to create session record", err);
    }
